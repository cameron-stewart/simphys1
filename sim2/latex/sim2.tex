...
\end{figure}
\FloatBarrier
Furthermore we can take a look at the total energy of the simulated system in Fig \ref{fig:energy}. As you can see the energy varies heavily when two particles collide, but after that the energy quickli returns to its former value. All in all the energy stays constant over time just like we expected it from the algorithm.
\begin{figure}[h]
\includegraphics[width=0.7\linewidth]{../fig/energy.png}
  \centering
  \caption{Energy of the system with modified initial conditions over time.}
\label{fig:energy}
\end{figure}
...
\end{cases}
\end{equation*}
 \listfile{../src/ljlib.py}{ljlib.py}{21}{35}{LJ Cutoff}{cuttoff}
We use a value of $2.5$ for $r_\mathrm{cutoff}$.
\subsubsection{Minimum Image Convention}
If we use a cutoff range that is less than the length of our box $L$ then we only have to compute the interaction between the closest image of the particles. All other images will be too far to interact. Knowing this, we can implement periodic boundary conditions by simply calculating the minimum image distance and using that distance to calculate the forces. Our particles will then freely propogate out of the box while still experiencing the correct force. When looking at the trajectory in VMD or in python we can then simpy wrap the particle locations back to the first box to see the true trajectories. The following code block shows how the minimum image is calculated:
 \listfile{../src/ljbillards1.py}{ljbillards1.py}{54}{57}{Minimum Image}{image}
 Thereby the length $L$ is defined elsewhere in the script as $L = 10.0$.
 The compute\_forces function was modified as well in order to include the functions image and the compute\_cutoff\_force. The function compute\_energy is extended in the same way.
  \listfile{../src/ljbillards1.py}{ljbillards1.py}{5}{19}{PBC Forces}{pbcforces}
We tested the periodic boundary conditions by setting up two particles with the following initial conditions:
 \listfile{../src/ljbillards1.py}{ljbillards1.py}{67}{75}{Initial Conditions}{init}
The trajectory can be seen in Fig. \ref{fig:pbc}. 
\begin{figure}[h]
\includegraphics[width=0.7\linewidth]{../fig/pbcplot.png}
  \centering
  \caption{Billard trajectory with periodic boundary conditions for two particles (0 and 1).}
\label{fig:pbc}
\end{figure}
...
We now inherit a nice python program to simulate an LJ fluid with periodic boundary conditions. We have only to import {\tt ljlib.py}, compute the size of the system, and place the particles programatically as shown in the following code block.
 \listfile{../src/ljfluid.py}{ljfluid.py}{81}{88}{Particle Placement}{place}
...
Similar to the previous task we were given a template, this time in C++, to modify. In this case we needed to implement the compute\_energy() function.
 \listfile[myCStyle]{../src/ljfluid.cpp}{ljfluid.cpp}{73}{91}{Compute Energy}{computeenergy}
...
\includegraphics[width=0.7\linewidth]{../fig/fit.png}
  \centering
  \caption{Plot of computing time for the C++ implementation vs number of particles along with a quadratic fit.}
\label{fig:cpp}
\end{figure}
\subsection{Cython}
Finally we will use cython as an interface between python and C so that the time intensive code is written in C and the control is done with python. We copied the compute\_energy() function from the previous task into the supplied template, lines 10 and 21 through 24 were uncommented in {\tt cython1/lj.pyx}, and lines 84 through 87 were uncommented in {\tt cython1/ljfluid.py}. The simulation was run for $n = {5,10}$ and runtimes of 0.568 and 2.724 seconds respectively were recorded. Compared to pure C++ the lower $n$ had a slower runtime while the higher had a quicker runtime.
...
\newpage
\listfile[myCStyle]{../src/cython2/c_lj.cpp}{cython2/c\_lj.cpp}{89}{111}{Compute Energy Verlet}{computeenergyverlet}
We ran this simulation for $n = {5, 10, 15, 20}$ and got runtimes of 0.392, 1.732, 11.916, and 59.960 seconds respectively. This shows a clear advantage in runtimes over the previous methods. All recorded runtimes can be found in {\tt times.txt}.
